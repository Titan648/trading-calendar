...
# Only this Telegram user ID can manage channels and see /canaux
...
async def owner_only(update, msg: str = "⛔ Commande réservée à l'administrateur du bot\."):
...
async def fetch_economic_events(days_ahead: int = 7) -> list:
    ...
    try:
        ...
        filtered = []
        for e in events:
            country = e.get("country", "").upper().strip()
            impact  = str(e.get("impact", "")).strip()
            name    = e.get("event", "").lower()

            # Keep if: major country OR high impact OR key keyword
            is_major   = country in MAJOR_COUNTRIES
            is_high    = impact in ("high", "3", "2", "medium")
            is_keyword = any(kw in name for kw in HIGH_IMPACT_KEYWORDS)

            if is_major or is_high or is_keyword:
                filtered.append(e)
...
async def fetch_todays_news(count: int = 10) -> list:
    """Fetch only news published TODAY (since midnight UTC)."""
    if not FINNHUB_TOKEN:
        return []
    url = f"https://finnhub.io/api/v1/news?category=general&token={FINNHUB_TOKEN}"
    try:
        ...
        today_start = int(datetime.now(ZoneInfo("UTC")).replace(
            hour=0, minute=0, second=0, microsecond=0
        ).timestamp())

        todays = [
            item for item in news
            if item.get("datetime", 0) >= today_start
        ]
        ...
        return (filtered or todays)[:count]
    except Exception as ex:
        logger.error(f"Finnhub today news error: {ex}")
        return []


async def fetch_todays_events() -> list:
    """Fetch economic events that happened TODAY."""
    if not FINNHUB_TOKEN:
        return []
    today = date.today()
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            ...
            resp.raise_for_status()
            events = resp.json().get("economicCalendar", [])
        # Only major countries with actual results
        return [
            e for e in events
            if e.get("country", "").upper() in MAJOR_COUNTRIES
            and e.get("actual") not in (None, "", "null")
        ]
    except Exception as ex:
        logger.error(f"Finnhub today events error: {ex}")
        return []
...
def build_evening_recap(markets_msg: str, events: list, news: list) -> str:
    """Build a combined evening recap: markets + today events results + today news."""
    sections = [markets_msg]
    ...
    if events:
        ...
        for e in events[:8]:
            ...
            # Determine if result beat/missed forecast
            trend = ""
            try:
            ...
            nums = []
            if actual:
                nums.append(f"Réel: *{esc(str(actual))}{trend}*")
            if forecast:
                nums.append(f"Prévu: `{esc(str(forecast))}`")
            if prev:
                nums.append(f"Préc: `{esc(str(prev))}`")
    ...
    sections.append("\n📌 _Sources: exchange\\-calendars · Finnhub_")
    return "\n".join(sections)
...
async def send_digest(ctx, is_morning: bool):
    ...
    for chat_id, mics in targets.items():
        try:
            if is_morning:
                # 🌅 MATIN: statut + événements semaine + actualités
                await ctx.bot.send_message(
                    chat_id=chat_id,
                    text=build_status_message(mics, "🌅 Digest Matin"),
                    parse_mode="MarkdownV2",
                )
                if events:
                    await ctx.bot.send_message(
                ...
                if news:
                    await ctx.bot.send_message(
                        chat_id=chat_id,
                        text=build_news_message(news, "📰 Actualités du matin"),
                        parse_mode="MarkdownV2",
                        disable_web_page_preview=True,
                    )
            else:
                # 🌆 SOIR: recap combiné en UN seul message
                markets_msg = build_status_message(mics, "🌆 Bilan de clôture")
                recap = build_evening_recap(markets_msg, events, news)
                await ctx.bot.send_message(
                    chat_id=chat_id,
                    text=recap,
                    parse_mode="MarkdownV2",
...
async def morning_digest_job(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    key   = f"morning_{today}"
    if _sent_today.get(key):
        logger.info("Digest matin déjà envoyé aujourd'hui — ignoré.")
        return
...
async def evening_digest_job(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if _sent_today.get(key):
        logger.info("Digest soir déjà envoyé aujourd'hui — ignoré.")
        return
    _sent_today[key] = True
    # Override: evening uses today-specific fetchers
    subs     = get_subscribers()
    channels = get_channels()
    targets  = {cid: mics for cid, mics in subs.items()}
    for ch in channels:
    ...
    logger.info(f"Digest soir: envoi à {len(targets)} destinataire(s).")
    events = await fetch_todays_events() if FINNHUB_TOKEN else []
    news   = await fetch_todays_news()   if FINNHUB_TOKEN else []
